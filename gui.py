...
def main(PCcriteria):
    ...
    n = 5
    PCcriteria
    #PCcriteria = np.array([[1, 2, 0.5, 3, 2], [0.5, 1, 2, 4, 0.5], [2, 0.5, 1, 0.2, 3], [1/3, 0.25, 5, 1, 2], [0.5, 2, 1/3, 0.5, 1]])
    ...
    #PCM1 = np.array([[1,2,5], [0.5,1,3], [0.2, 1/3, 1]])
    #PCM2 = np.array([[1, 3, 4], [1/3, 1, 3], [0.25, 1/3, 1]])
    #PCM3 = np.array([[1, 2, 3], [0.5, 1, 2], [1/3, 1/2, 1]])
    #PCM1 = np.array([[1, 2, 0.5, 3, 2], [0.5, 1, 0.2, 0.5, 2], [2, 5, 1, 3, 0.5], [1/3, 2, 1/3, 1, 1], [0.5, 0.5, 2, 1, 1]])
    #PCM2 = np.array([[1, 0.5, 2, 0.25, 3], [2, 1, 3, 0.5, 4], [0.5, 1/3, 1, 2, 0.5], [4, 2, 0.5, 1, 2], [1/3, 0.25, 2, 0.5, 1]])
    #PCM3 = np.array([[1, 3, 0.5, 2, 4], [1/3, 1, 2, 0.25, 0.5], [2, 0.5, 1, 2, 3], [0.5, 4, 0.5, 1, 1], [0.25, 2, 1/3, 1, 1]])
    #PCM4 = np.array([[1, 1, 2, 3, 2], [1, 1, 0.5, 2, 4], [0.5, 2, 1, 0.25, 2], [1/3, 0.5, 4, 1, 2], [0.5, 0.25, 0.5, 0.5, 1]])
    ...
    for i in winners:
        ...
        exec(f'var_a{j}.set(suggestions[j])')
        ...
        exec(f"var_f{j}.set(str(list(config['devices'][index].items())[0][1]['perf']) + ' points')")
...
def showTooltip(content, parent, relx, rely, width, bg, fg):
    #ToolTip megmutatásához használható
    global ttp_lbl
...
def hideTooltip():
    #ToolTip elrejtéséhez használható
    ttp_lbl.configure(text="", bg="#f0f0f0", relief=FLAT, bd=0)
...
def saveFile():
    #yFile = filedialog.asksaveasfile(mode="w", defaultextension=".txt")
    #if myFile is None:
    #    return
    #content = proba.get(1.0, "end-1c")
    #content2 = proba2.get(1.0, "end-1c")
    #myFile.write(content + " and " + content2)
    names = [name0, name1, name2]
    print(names)
    ##proba = Text(root)
    #proba.insert(END, "Hello there dude")
    ###proba.place(relx=0.9, rely=0.9)
    #proba2 = Text(root)
    #proba2.insert(END, "Hi mama")x
    #saveFile()
...
#frame_proba= Frame(root)
#frame_proba.place(x=0, y=0)
#frame_proba.configure(bg="#DF1176", width=900, height=100)
...
#Üdvözlő szöveg
title_lbl = Message(root, text="Kérem, hasonlítsa össze egymással az alábbi szempontokat! Csak egész számokat, illetve tizedesponttal elválasztott tizedestörteket használjon!")
...
#empty_label.configure(background="#c3dbd0")
...
for k in range(10):
    ...
    if tmp.startswith('Ár'):
        ...
        exec(f'img_lbl_a{k}.bind("<Enter>", lambda e : showTooltip(price_info, root, 0.38, 0.83, 110, "white", "#DF1176"))')
    elif tmp.startswith('Kamera'):
        exec(f'img_lbl_a{k} = Label(frame1, image = camera_icon)')
        exec(f'img_lbl_a{k}.bind("<Enter>", lambda e : showTooltip(camera_info, root, 0.38, 0.83, 110, "white", "#DF1176"))')
    elif tmp.startswith('Tárhely'):
        exec(f'img_lbl_a{k} = Label(frame1, image = storage_icon)')
        exec(f'img_lbl_a{k}.bind("<Enter>", lambda e : showTooltip(storage_info, root, 0.38, 0.83, 110, "white", "#DF1176"))')
    elif tmp.startswith('Akkumulátor'):
        exec(f'img_lbl_a{k} = Label(frame1, image = battery_icon)')
        exec(f'img_lbl_a{k}.bind("<Enter>", lambda e : showTooltip(battery_info, root, 0.38, 0.83, 110, "white", "#DF1176"))')
    else:
        exec(f'img_lbl_a{k} = Label(frame1, image = performance_icon)')
        exec(f'img_lbl_a{k}.bind("<Enter>", lambda e : showTooltip(performance_info, root, 0.38, 0.83, 110, "white", "#DF1176"))')
    ...
    if tmp.endswith('Ár'):
        ...
        exec(f'img_lbl_b{k}.bind("<Enter>", lambda e : showTooltip(price_info, root, 0.38, 0.83, 110, "white", "#DF1176"))')
    elif tmp.endswith('Kamera'):
        exec(f'img_lbl_b{k} = Label(frame1, image = camera_icon)')
        exec(f'img_lbl_b{k}.bind("<Enter>", lambda e : showTooltip(camera_info, root, 0.38, 0.83, 110, "white", "#DF1176"))')
    elif tmp.endswith('Tárhely'):
        exec(f'img_lbl_b{k} = Label(frame1, image = storage_icon)')
        exec(f'img_lbl_b{k}.bind("<Enter>", lambda e : showTooltip(storage_info, root, 0.38, 0.83, 110, "white", "#DF1176"))')
    elif tmp.endswith('Akkumulátor'):
        exec(f'img_lbl_b{k} = Label(frame1, image = battery_icon)')
        exec(f'img_lbl_b{k}.bind("<Enter>", lambda e : showTooltip(battery_info, root, 0.38, 0.83, 110, "white", "#DF1176"))')
    else:
        exec(f'img_lbl_b{k} = Label(frame1, image = performance_icon)')
        exec(f'img_lbl_b{k}.bind("<Enter>", lambda e : showTooltip(performance_info, root, 0.38, 0.83, 110, "white", "#DF1176"))')
    ...
    #exec(f'E{k}.bind("<Enter>", lambda e : print("hello"))')
...
#root.configure(bg="#c3dbd0")
#root.configure(bg="#DF1176")
